...
const storage = multer.diskStorage({
    ...
    filename: (req, file, cb) => {
        cb(null, 'supplier_' + Date.now() + path.extname(file.originalname));
    }
});
const upload = multer({ storage: storage });

const editSupplierProfile = [upload.single('image'), async (req, res) => {
    try {
        ...
        const currentAdmin = await prisma.supplier.findUnique({
            where: { id: Number(supplierId) },
            select: { companyName: true, email: true, image: true, password: true }
        });
        ...
        if (email && email !== currentAdmin.email) {
            const existingAdmin = await prisma.supplier.findFirst({
                where: {
                    email: email,
                    NOT: { id: Number(supplierId) }
                }
            });
        ...
        await prisma.supplier.update({
            where: { id: Number(supplierId) },
            data: {
                companyName: companyName || currentAdmin.companyName,
...
module.exports = { editSupplierProfile };
